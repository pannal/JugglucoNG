...
static uint8_t challenge_table_byte(const l3_challenge_block_tables *t, uint32_t addr) {
    ...
    /* A malformed or incomplete recovered table must fail the handshake, not
     * terminate the glucose-monitoring process. Returning a deterministic bad
     * byte makes the authenticated challenge fail closed downstream. */
