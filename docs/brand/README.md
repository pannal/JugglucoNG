...
The renderer reads local files only and writes the banner, previews and Android resources. The monochrome pen has fewer details so it remains legible when tinted. The master frame follows the blood drop's centerline. The drop and horns are symmetric about that line; adding a pen or badge does not move the anchor. All launcher variants share a scale that keeps the full artwork inside the safe circle. Adaptive foregrounds use a 108 dp layer with the mark centered in its 66 dp safe region, following [Android's icon guidance](https://developer.android.com/develop/ui/compose/system/icon_design_adaptive).
